    ...
    def build(self,*args):
        ...
        if  os.path.exists(filename):
            ...
            try:
                ...
                db_mbtiles = sqlite3.connect(filename)
                db_mbtiles.close()
                req1 = UrlRequest("http://localhost:5000/tiles")
                req1.wait()
                self.fillTileTable(req1)
                req2 = UrlRequest("http://localhost:5000/metadata")
                req2.wait()
                self.fillMetaDataTable(req2)
                req3 = UrlRequest("http://localhost:5000/route/info")
                req3.wait()
                self.fillRouteInfoTable(req3)
                req4 = UrlRequest("http://localhost:5000/route/coordinates")
                req4.wait()
